# -*- coding: utf-8 -*-
"""QGIS plugin admin operations for SG Diagram Downloader"""
...
def copy_source_files(
    ...
    # Copy Python source files from root
    source_files = [
        "custom_logging.py",
        "__init__.py",
        "database_manager.py",
    ...
    if resources_dir.exists():
        shutil.copytree(str(resources_dir), str(output_directory / "resources"))
...
def _get_metadata() -> typing.Dict:
    """Reads the metadata properties from the metadata.txt file

    :return: plugin metadata
    :type: Dict
    """
    config_path = LOCAL_ROOT_DIR / "metadata.txt"
    config = configparser.ConfigParser()
    config.read(config_path)

    general_metadata = dict(config["general"])
...
def _get_existing_releases(
    ...
    """
    base_url = "https://api.github.com/repos/kartoza/sg-diagram-downloader/releases"
